...
if (!process.env.GEMINI_MODEL || process.env.GEMINI_MODEL.includes('2.5') || process.env.GEMINI_MODEL.includes('2.0') || process.env.GEMINI_MODEL.includes('1.5')) {
...
function markModelCooldown(model: string, err: any) {
  ...
  modelCooldownMap.set(model, Date.now() + cooldownSec * 1000);
}
...
app.post('/api/chat/title', async (req: Request, res: Response) => {
  try {
    ...
    for (const model of getOrderedCandidates()) {
      try {
        const result = await ai.models.generateContent({
          model,
          contents: `Provide a short title (3-5 words, no quotes) for a chat beginning with: "${firstLine.slice(0, 120)}"`,
        });

        if (result.text) {
          res.json({
            title: result.text.trim().replace(/^["'#*]+|["'#*]+$/g, '').slice(0, 45),
          });
          return;
        }
      } catch (err: any) {
    ...
    res.json({
      title: firstLine.split(/\s+/).slice(0, 5).join(' ') || 'New Conversation',
    });
  } catch {
    res.json({ title: 'New Conversation' });
  }
...
app.post('/api/chat/stream', async (req: Request, res: Response) => {
  ...
  try {
    const { messages, customSystemInstruction, mode } = req.body;
    ...
    if (!streamResponse) {
      throw lastError || new Error('All model candidates are unavailable.');
    }
    ...
    if (!isAborted) {
      res.write(
        `data: ${JSON.stringify({ error: extractCleanErrorMessage(error) })}\n\n`
      );
      res.end();
    }
  }
